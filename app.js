...
const MODES = {
  ...
  'county-hard': { group:'county', check:true, title:'Megye tanulás – nehezített', badge:'Megye · nehezített', text:'Helyezd el az összes megyenevet, majd kattints az "Ellenőrzés" gombra. A hibás válaszok ABC sorrendben térnek vissza a listára.' },
  'seat-easy': { group:'seat', check:false, showLabels:true, title:'Megyeszékhely tanulás – könnyített', badge:'Székhely · könnyített', text:'A megyenevek látszanak a térképen. Kattints egy megyeszékhely nevére, majd a hozzá tartozó piros pontra. A jó válasz azonnal a helyére kerül.' },
  'seat-medium': { group:'seat', check:false, showLabels:false, title:'Megyeszékhely tanulás – nehezített', badge:'Székhely · nehezített', text:'A megyenevek most nem látszanak. Kattints egy megyeszékhely nevére, majd a térképen a megfelelő pontra. A jó válasz azonnal a helyére kerül.' },
  'seat-extreme': { group:'seat', check:true, showLabels:false, title:'Megyeszékhely tanulás – extrém', badge:'Székhely · extrém', text:'A megyenevek nem látszanak. Helyezd el az összes megyeszékhelyet, majd kattints az "Ellenőrzés" gombra. A hibásak ABC sorrendben térnek vissza.' }
};
...
function applyZoom() { const { scale, x, y } = state.zoom; el.zoomLayer.style.transform = `translate(${x}px,${y}px) scale(${scale})`; el.zoomLayer.querySelectorAll('.chip.placed').forEach(chip => { chip.style.transform = `translate(-50%,-50%) scale(${1 / scale})`; }); el.zoomReset.hidden = scale <= 1.02; }
...
function onTouchStart(event) { if (event.touches.length === 2) { event.preventDefault(); const mid = relative(touchPoint(event.touches[0], event.touches[1])); state.pinch = { distance: distance(event.touches[0], event.touches[1]), scale: state.zoom.scale, anchor: { x: (mid.x - state.zoom.x) / state.zoom.scale, y: (mid.y - state.zoom.y) / state.zoom.scale } }; } }
...
function buildMap() { el.map.replaceChildren(); state.counties.forEach(county => { const polygon = document.createElementNS(SVG_NS, 'polygon'); polygon.setAttribute('points', county.points); polygon.classList.add('region'); polygon.dataset.id = county.id; polygon.addEventListener('click', () => onRegionClick(county)); el.map.appendChild(polygon); }); }
...
function addChip(text, data) { const chip = document.createElement('button'); chip.className = 'chip'; chip.type = 'button'; chip.textContent = text; chip.dataset.id = data.id; chip._data = data; chip.addEventListener('click', event => { event.stopPropagation(); selectChip(chip); }); el.trayItems.appendChild(chip); return chip; }
...
function startMode(key) { state.mode = key; state.score = 0; state.selectedChip = null; resetZoom(); updateScore(); el.trayItems.replaceChildren(); buildMap(); const current = mode(); el.check.hidden = !current.check; el.instructions.textContent = `${current.title}: ${current.text}`; el.badge.textContent = current.badge; el.menuScreen.hidden = true; el.gameScreen.hidden = false; state.counties.forEach(county => { if (current.group === 'county') { const region = findRegion(county.id); el.trayItems.appendChild(addChip(county.id, { id: county.id })); } else { addSeatMap(county, current.showLabels); if (county.seat) el.trayItems.appendChild(addChip(county.seat, { id: county.seat })); } }); }
function showMenu() { el.overlay.classList.remove('show'); state.mode = null; state.selectedChip = null; resetZoom(); el.zoomLayer.replaceChildren(); el.map.replaceChildren(); el.trayItems.replaceChildren(); el.instructions.textContent = ''; el.check.hidden = true; el.gameScreen.hidden = true; el.menuScreen.hidden = false; }
function checkAnswers() { [...el.zoomLayer.querySelectorAll('.chip.placed:not(.locked)')].forEach(chip => { const correct = chip._pending === chip._data.id; const county = mode().group === 'county' ? state.counties.find(item => item.id === chip._data.id) : state.counties.find(item => item.seat === chip._data.id); if (correct) { lockChip(chip, mode().group === 'county' ? county.labelPos : county.seatPos, mode().group === 'county' ? findRegion(county.id) : findSeat(county.seat)); state.score += 10; } else setTimeout(() => returnTray(chip), 200); }); updateScore(); setTimeout(complete, 300); }

el.mapWrap.addEventListener('touchstart', onTouchStart, { passive:false }); el.mapWrap.addEventListener('touchmove', onTouchMove, { passive:false }); el.mapWrap.addEventListener('touchend', onTouchEnd); el.zoomReset.addEventListener('click', resetZoom); el.check.addEventListener('click', checkAnswers); el.restart.addEventListener('click', () => state.mode && startMode(state.mode)); el.menuBtn.addEventListener('click', showMenu); document.querySelectorAll('.mode-button').forEach(button => button.addEventListener('click', () => startMode(button.dataset.mode)));
